...
const sofaSensorState = msg.payload['binary_sensor.presence_sensor_fp2_c63f_presence_sensor_5']; 
...
if (sofaSensorState === 'on') {
    ...
    if (tvState === 'on' && (currentHour >= 17 || currentHour < 8)) {
        ...
        node.status({ fill: "yellow", shape: "dot", text: "電視開啟模式 (17:00-07:59)" });
    }
    // 電視機未開啟時的燈光判斷
    else {
        // outputs[0]: 電視機未開啟，日常時段有人觸發 ( 08:30 ~ 01:59 )
    ...
    node.status({ fill: "red", shape: "dot", text: "客廳沙發無人 - 30 秒後嘗試關燈" });
    // outputs[3]: 無人觸發 (延遲 30 秒防抖動)
